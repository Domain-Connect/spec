...
hen constructing a deep link using `"urlControlPanel"`, the Service Provider MUST replace any occurrence of the literal token `"%domain%"` with the percent-encoded ACE-form domain name per <<RFC3986>>. The `"%domain%"` token MUST be the only substitution token used.
...
- Created Security Considerations sections and moved relevant part in there.
