...
Not all templates are susceptible; the risk is proportional to how much of a record's content is controlled by variables. Three mitigations are available to template authors and Service Providers: disabling the synchronous flow via `"syncBlock"` (see <<sec-sync-blocking,format=title>>), digitally signing apply requests (see the Section <<sec-signing,format=title>>), and setting the `"warnPhishing"` flag to prompt the DNS Provider to display additional warnings to the user (see <<sec-warnings,format=title>>).
