...
When used in a template definition, the value MUST conform to the `dc-pointsto-tmpl` syntax (see <<terminology>>). +
...
None, All, or Prefix. <<record-types-conflicts, See below>>. +
...
|The prefix to detect conflicts when txtConflict-MatchingMode is `"Prefix"`. <<record-types-conflicts, See below>>. +
...
If the request is valid, the DNS Provider authenticates the user, validates user's authorization to modify the DNS zone, obtains authorization to apply the templeate to the DNS and finally applies the changes. This process is described in <<template-application>>.
...
After succesful processing of Template Apply Request the DNS Provider MUST terminate the user flow according to one of the following cases:
...
If the DNS Provider cannot complete the apply operation — for example because authentication failed, the user does not control the domain, the domain is suspended, or the user explicitly cancelled — the DNS Provider MUST signal an error.
...
|instanceId
|(OPTIONAL) Only applicable to templates supporting multiple instances
(see <<template-definition, multiInstance>> template property). When present, only the identified instance MUST be removed; if absent, all instances of the template MUST be removed. +
...
To minimise delays caused by DNS resolver caching, it is RECOMMENDED that the Service Provider query a DNS resolver configured with low TTL overrides, or query the authoritative name servers directly.
...
//TODO: solve ambiguity in `"@"` for host and pointsTo/data.
//TODO: review RR definitions (and template repo) why/if we have to refer to both host/name and pointsTo/data.
...
. *Perform conflict detection* — The DNS Provider checks the resolved RR set against the existing zone content according to the conflict detection rules (see <<record-types-conflicts>>).
...
. *Apply changes* — After authorization is granted, or at later point in Asynchronous flow:
  ...
  .. (olny DNS Providers tracking template state): Record the new template instance
...
A DNS Provider not maintainig applied template state, applying a template whose records conflict with any existing record in the zone MUST remove all those conflicting records.
...
- Added comprehensive ABNF grammar to the Terminology section, providing normative syntax definitions for all protocol identifiers, domain name forms, template fields, OAuth parameters, and signing artifacts.
- Clarify explicit IDN support for domain name parameters (undefined before).
- Replaced informal prose constraints across all endpoint parameter tables and template field definitions with normative ABNF references
- Public-key TXT record format replaced with a formal ABNF grammar and detailed specification.
- Restructured Template Application: replaced ad-hoc conflict sections with focused Conflict Detection, User Authorization of Changes, and Template Application sections; added Essential Records definition and Steps to Apply a Template.
- Removed non-normative content: Public Template Repository, General Considerations, and Extensions/Exclusions sections.
- Moved display-related and UX behavioural requirements out of field definitions into <<user_authorization_of_changes,format=title>>; reworded developer-only fields to express intent rather than normative prohibition.
- Moved runtime processing rules out of parameter and template field definitions into their appropriate processing sections.
- <<security-considerations,format=title>> section restructured: phishing threat description moved from <<synchronous-flow,format=title>> into new <<sec-variable-phishing,format=title>> subsection; editorial cleanup including section reordering, heading renames, and example block formatting.
- Added <<group-filtering,format=title>> subsection to <<template-application,format=title>>.
- Restructured request signing description into three focused sections: <<signing-procedure,format=title>>, <<sig-verification,format=title>> and <<pubkey-publication,format=title>>.
- Replaced Sync Apply Interaction with a normative three-part structure (Request, Response, Error Response)
- Replaced all occurrences of the term "browser" with "user agent" throughout the document.
- Rewrote <<verification-of-changes,format=title>> as normative text.
- Clarified the meaning of "synchronous" in <<synchronous-flow,format=title>>: the term describes the user-interaction model (user present, single session) and does not imply immediate DNS propagation; added normative note directing Service Providers to <<verification-of-changes,format=title>>.
- Defined an extensibility model for the <<dns-provider-discovery,format=title>> settings response: unknown properties MUST be ignored by clients; DNS Providers MAY add properties; all properties are registered in a new IANA "Domain Connect Settings Properties" registry with Specification Needed policy.
- Added ABNF rule defining the normative syntax for `"properties"` parameter values across the <<apply-template-params,format=title>>, <<oauth-authorization-request,format=title>>, and <<oauth-apply-template,format=title>> parameter tables.
