...
Each group identifier MUST conform to the `dc-id` syntax (see <<terminology>>).
Multiple groups MAY be specified as a comma-separated list of `dc-id` values.
TODO: define an own term for a comma-separated list of groupIds
...
The value MUST conform to the `dc-alg-id` rule above and MUST be registered in the
IANA "JSON Web Signature and Encryption Algorithms" registry established by <<RFC7518>>. +
...
|*State*
|state
|(OPTIONAL) This is a random, unique string passed along to prevent CSRF or to pass state value back to the caller.

The value MUST conform to the `"state"` syntax (see <<terminology>>).

If present it MUST be returned as a parameter appended to the `redirect_url` described above.
