...
- Added comprehensive ABNF grammar to the Terminology section, providing normative
  syntax definitions for all protocol identifiers, domain name forms, template fields,
  OAuth parameters, and signing artifacts.
- Added IDN support for domain name parameters; infrastructure fields restricted to
  ACE-form only.
- Replaced informal prose constraints across all endpoint parameter tables and template
  field definitions with normative ABNF references; public-key TXT record format
  replaced with a formal ABNF grammar.
- Restructured Template Application: replaced ad-hoc conflict sections with focused
  Conflict Detection, User Authorization of Changes, and Template Application sections;
  added Essential Records definition and Steps to Apply a Template.
- Added Identifier Syntax subsection defining normative constraints on identifier fields
  across all endpoints and template records.
- Removed non-normative content: Public Template Repository, General Considerations,
  and Extensions/Exclusions sections.
