...
If the template's `"syncBlock"` field is `"true"`, the DNS Provider MUST NOT process the request and MUST return an error.
...
The API MUST validate the access token, and that the domain belongs to the user and is represented by the token being presented. The `"domain"` and `"host"` values MUST match those that were authorized in the access token. Further it MUST be validated that the template identified by `"providerId"`/"`serviceId`" and optionally `"instanceId"` exists and has been applied to the `"domain"`/`"host"` by the Service Provider, or an error response with code 410 SHOULD be returned that the call would have no effect.

The Service Provider MUST remove all still active DNS Resource Records belonging to this template instance. 
...
Template fields of Resource Records in active groups MUST be processed - meaning where `"groupId"` matches any of group identifiers specified in the template application, or all groups if no group indentifiers were specified. Template fields of Resource Records in inactive groups MUST NOT be processed.
...
- Moved runtime processing rules out of parameter and template field definitions into their appropriate processing sections: redirect and state handling into <<browser-handling,format=title>>, ignored-parameter rule into <<variable-substitution,format=title>>, and synchronous-block enforcement into <<apply-template,format=title>>; field definitions for `"syncPubKeyDomain"`, `"syncRedirectDomain"`, and `"multiInstance"` reworded to describe the field's meaning only.
- Removed conditional processing semantics from parameter table cells: `"host"` and `"domain"` cells across sync and OAuth flows reworded to describe field meaning only; token-matching rule moved to <<oauth-apply-template,format=title>>; `"groupId"` default-all behaviour moved to <<steps-to-apply,format=title>>; `"code"`, `"refresh_token"`, and `"redirect_uri"` token request cells rewritten with field-level mutual-exclusivity constraints only; cross-field request validation rules consolidated into a normative paragraph in <<oauth-token-request,format=title>>.
- Added <<host-name-rendering,format=title>> subsection to <<variable-substitution,format=title>> specifying how template `"host"` field values are resolved against the `"domain"` and `"host"` apply parameters; added <<template-scope,format=title>> and Subdomains subsections to a new Template Considerations section, replacing the former Variables and Host/Name in Template section; removed general recommendation against variables in `"host"` fields.
