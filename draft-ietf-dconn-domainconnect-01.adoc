...
When present, the value MUST be a non-negative integer.
...
|The prefix to detect conflicts when txtConflict-MatchingMode is "Prefix".
The value MUST be a non-empty string consisting of printable ASCII characters only.
It MUST NOT contain variable expressions. <<record-types-conflicts, See below>>.
...
- Added normative constraint on `version`: MUST be a non-negative integer when present.
