...
|width
|Number
|(OPTIONAL) This is the desired width of the window for granting consent when navigated in a popup. Default value if not returned is 750px.
The Service Providers SHOULD obey to this setting if opening a pop-up window to assure optimal display of the user interface of the DNS Proviver.

|*Height of Window*
|height
|Number
|(OPTIONAL) This is the desired height of the window for granting consent when navigated in a popup. Default value if not returned is 750px.
The Service Providers SHOULD obey to this setting if opening a pop-up window to assure optimal display of the user interface of the DNS Proviver.
...
This field allows a Service Provider whose template isn't supported at the DNS Provider to provide a direct link to perform manual edits.

To allow deep links to the specific domain, this string MAY contain the literal token `"%domain%"` which MUST be replaced with the percent-encoded ACE-form domain name per <<RFC3986>> when building the deep link to the control panel of a given domain.
`"%domain%"` MUST be the only substitution token permitted in this field.

After potentially substituting the `"%domain%"` token this MUST be a valid URI <<RFC3986>> with scheme `"https"`.

|*Name Servers*
|nameServers
|List of String
|(OPTIONAL) This is the list of nameserver hostnames held by the DNS Provider in the zone (typically based on NS entries). Service Provider MAY use this list to compare with authoritative list of nameservers obtained from the registry or authoritative NS query, so ensure that the actual delegated server has been reached. A mismatch may be a result of stalled or error `"_domainconnect"` record, or a deliberate action of DNS adminstrator, for example in order to pre-provision the service with a new DNS provider. +
...
|*Host*
|host
|(OPTIONAL) This is the host name of the Sub Domain.
If left blank, the template is being applied to the Zone Apex.
Otherwise the template is applied to the sub domain of the domain in the same zone. +

When present, the value MUST be a single name conforming to `domain-name` (see <<terminology>>) or an empty string.

|*Redirect URI*
|redirect_uri
|(OPTIONAL) The location to direct the client browser to upon successful authorization, or upon error. +

The value MUST be an absolute URI conforming to <<RFC3986>>. +

If omitted the DNS Provider SHOULD close the browser window upon completion. It MUST be scoped to the `"syncRedirectDomain"` authority from the template, or the request MUST be signed.
...
|*Service Name*
|serviceName
|(OPTIONAL) This parameter allows for the caller to provide additional text for display with the template `"serviceName"`. It SHOULD be used to augment the `"serviceName"` value from the template, not replace it. This parameter is only allowed when the `"sharedServiceName"` attribute is set in the template. +
...
If no group is specified, all groups are applied. +
...
See the Security Considerations section below for the signing procedure.
...
* Error
+
If authorization could not be obtained or an error has occurred, the parameter `"error="` must be appended. For consistency with the asynchronous OAuth flows the valid values for the error parameter will be as specified in OAuth 2.0 <<RFC6749>> (4.1.2.1. Error Response - `"error"` parameter). Valid values are: `"invalid_request"`, `"unauthorized_client"`, `"access_denied"`, `"unsupported_response_type"`, `'invalid_scope'`, `"server_error"`, and `"temporarily_unavailable"`.
...
//TODO: move it to own section describing mechanism
...
The Service Provider MUST publish their public key in one or more DNS TXT records
under the host specified by the `"key"` parameter within the domain given by
...
Multiple TXT records at the same host are used to split a large key across DNS
record size limits; they MUST be reassembled in ascending order of the `"p"`
(fragment index) field.
...
|*Fragment Payload*
|d
|(REQUIRED) The base64-encoded key material for this fragment. +
...
Signing the query string by the Service Provider is OPTIONAL. Not all Services Provider templates require or are able to provide this level of security. 
Presence of the `"syncPubKeyDomain"` in the template indicates that the template requires signature verification.
...
For example, most services are sold and provided by the same company. However, some Service Providers have a reseller channel. This allows the service to be provided by the Service Provider, but sold through third parties.
...
While the variables for the applied template would be provided later, the values of some variables may be necessary to determine conflicts. As such, any variables impacting conflicting records SHOULD be provided in the consent flow. This primarly includes variables in hosts, and variables in the data portion for certain TXT records.
...
Each entry MUST be either an empty string (representing the Zone Apex) or a name conforming to `domain-name`.
An empty entry in the list (e.g., a leading or trailing comma) represents the Zone Apex.
...
If present, it MUST be returned as a parameter appended to the `”redirect_uri”` described above.
...
When the secret is provided (which is the normal case), care must be taken. A malicious user could create a domain that returns a false _`"domainconnect"` TXT record, and subsequently a JSON call to their own server for the API end point. By doing so, they could then run Domain Connect on their domain and retrieve the secret.
...
|*Authorization Code*
|code
|(OPTIONAL) The authorization code returned in the authorization response when the user accepted the authorization request.
REQUIRED for initial `"access_token"` request. This value MUST NOT be set for `"access_token"` refresh request. +

The value MUST conform to the `"code"` syntax as defined in Appendix A, Section A.11 of <<RFC6749>>.

|*Refresh Token*
|refresh_token
|(OPTIONAL) The refresh token used to obtain a new access token when the current one has expired.
REQUIRED for `"access_token"` refresh request. This value MUST NOT be set for initial `"access_token"` request.+

The value MUST conform to the `"refresh_token"` syntax as defined in Appendix A, Section A.17 of <<RFC6749>>.

|*Redirect URI*
|redirect_uri
|(CONDITIONAL) REQUIRED if a `"redirect_uri"` was included in the authorization request.
When present, it MUST be identical to the `"redirect_uri"` used in that request. +
...
|*Group ID*
|groupId
|(OPTIONAL) Specifies the group of changes in the template to apply. If omitted, all changes are applied.
The value MUST conform to the `dc-id-list` syntax (see <<terminology>>).
...
|*Domain*
|domain
|(REQUIRED) The Zone Apex domain name being configured. It MUST match the domain that was authorized in the token. +
...
|Boolean
|syncBlock
|(OPTIONAL) Indicates that the synchronous protocol MUST be disabled for this template.  +
...
|sharedProviderName
|(OPTIONAL)
This flag indicates that the template allows the caller to pass in additional information for the `"providerName"`. This information SHOULD augment the display of the `"providerName"` from the template.  +

The default for this is false.  +

For backward compatability with DNS Providers not at V2.2 of the spec it is recommended that the shared flag also be set.
...
|syncRedirectDomain
|(OPTIONAL)
When present, this is a comma-separated list of domain names for which redirects are allowed to be sent after applying a template for the synchronous flow. DNS Provider MUST NOT send redirects to any other domain name not included on this list. +
...
|multiInstance
|(OPTIONAL)
When set to true, it indicates that the template MAY be applied multiple times. This only impacts DNS Providers that maintain template state in DNS.
...
[[variables]]
=== Resource Record and Variable Processing
==== Variable Syntax
...
. *Resolve variables* — All variable expressions in the template records are substituted with the values provided by the caller, producing a concrete set of DNS resource records (RRs) to be applied.
...
. *Pre-calculate conflict resolution* — Determine the full set of records and,
  where applicable, template instances that would be removed upon application.
...
. *Apply changes* — After authorization is granted:
...
. *Redirect browser* — If a `redirect_uri` parameter was provided in the request, the DNS Provider MUST redirect the user agent to that URI upon successful template application (see <<browser-handling>>).
...
Records with `"essential=OnApply"` MUST be applied when the template is first applied, but MAY be subsequently removed or altered — by the user or through the application of another template — without triggering conflict detection or removal of the owning template.
...
Manual changes made by the user at the DNS Provider MAY also trigger conflict detection against applied templates.
...
If conflicts are detected — at either the template or record level — the DNS Provider SHOULD present these to the user before authorization is granted. For templates this means identifying services that would be disabled; for records this means identifying records that would be deleted or overwritten.
...
After authorization is granted, the DNS Provider applies the template to the DNS zone.
...
- Added Essential Records subsection under Template Considerations, reformulated from non-essential records concept to a definition-style description of essential records.
- Replaced Disclosure of Changes and Conflicts and Record Types and Conflicts sections with three focused sections: Conflict Detection, User Authorization of Changes, and Template Application.
- Added Steps to Apply a Template overview section with ordered steps and branching for DNS Providers tracking template state vs. those that do not.
- Added normative ABNF syntax rules to the Terminology section.
- Added Identifier Syntax subsection defining normative constraints on `providerId`, `serviceId`, `groupId`, and `instanceId` via the `dc-id` rule.
- Added IDN support for `domain` and `host` parameters across all endpoints: both Unicode (U-label/UTF-8) and ACE (A-label/punycode) forms are now accepted; receivers MUST normalise U-labels to A-labels per RFC 5891.
- Restricted `nameServers`, `syncPubKeyDomain`, and `syncRedirectDomain` to ACE-form domain names only (`dc-ascii-name`).
- Strengthened normative constraints on URL fields (`urlSyncUX`, `urlAsyncUX`, `urlAPI`, `urlControlPanel`, `logoUrl`) with explicit RFC 3986 conformance requirements.
- Specified `dc-sig` (base64 per RFC 4648) for the `sig` parameter and `dc-key-label` (single plain ACE or RFC 8552 underscore-prefixed label) for the `key` parameter in the synchronous signing flow.
- Specified `dc-force` for the `force` parameter (string `"0"` or `"1"`), `"state"` for the `state` parameter.
- Added normative display-name constraints (UTF-8, at most 255 code points, printable, no control characters) for `providerName`, `providerDisplayName`, and `serviceName`.
- Clarified `groupId` (in endpoint parameters and template records) must conform to `dc-id` and MUST NOT contain variable expressions.
- Added normative reference to `spfRules` field: must contain only SPF mechanism/modifier terms per RFC 7208 Section 5, excluding version prefix and `all` qualifier.
- Added normative constraint on `txtConflictMatchingPrefix`: printable ASCII only, no variable expressions.
- Added normative constraint on `version`: replaced prose "non-negative integer" constraint with the `dc-version` ABNF rule (positive integer, no leading zeros, no fractional or exponent part; strict subset of JSON `"number"` per RFC 8259 Section 6); added RFC 8259 as a normative reference.
- Added normative references: RFC 4648 (Base64), RFC 5891 (IDNA 2008).
- Added `dc-underscore-label` ABNF rule for RFC 8552 underscore-prefixed DNS labels; added `dc-key-label` rule (plain ACE or underscore label) replacing the former `dc-key-host` single-label rule for the `key` parameter; added `dc-pubkey-domain` rule allowing zero or more underscore-label prefixes on `syncPubKeyDomain`; updated normative constraints on `syncPubKeyDomain` and `key` parameter accordingly.
- Added `dc-display-name` ABNF rule referencing `unicode-assignable` (RFC 9839 Section 4.3) for display-name fields; replaced informal UTF-8/printable-characters prose constraints on `providerName` (DNS Provider discovery and template) and `serviceName` (template) with a normative reference to `dc-display-name`; added RFC 9839 as a normative reference.
- Added `dc-description-text` ABNF rule (at most 2048 `unicode-assignable` code points per RFC 9839 Section 4.3); added normative `dc-description-text` constraints to the `description` and `variableDescription` template fields.
- Added normative constraints to the Query Supported Template endpoint parameter table: `urlAPI` cross-referenced to the settings endpoint; `providerId` and `serviceId` required to conform to `dc-id` syntax.
- Removed redundant "Both ACE … forms are accepted; the receiver MUST convert …" sentences from `domain` and `host` parameter descriptions across all endpoints; the `domain-name` terminology reference is sufficient. Retained normative A-label normalisation requirements for token comparison in the OAuth apply and revert flows.
- Added normative constraints to the synchronous Apply Template parameter table: `urlSyncUX` cross-referenced to the settings endpoint; `providerId` and `serviceId` required to conform to `dc-id`; `redirect_uri` required to be an absolute URI per RFC 3986; `properties` parameter names required to conform to `variable-name` syntax with DNS Provider as the correct receiver of the ignore-unknown-parameters rule.
- Replaced informal prose description of the public key TXT record format in the Digitally Sign Requests section with a formal ABNF grammar (`dc-pubkey-record`, `dc-pubkey-kv`, `dc-frag-index`, `dc-frag-data`, `dc-alg-id`, `dc-key-type`); added normative constraints to all four TXT record fields (`"p"`, `"d"`, `"a"`, `"t"`) including syntax rules, mandatory reassembly order, default values, and IANA registry reference for algorithm identifiers.
- Added `dc-id-list` ABNF rule (comma-separated list of `dc-id` values) to Terminology; replaced ad-hoc prose in `groupId` parameter descriptions (synchronous apply and asynchronous apply) with normative `dc-id-list` reference; resolved pre-existing TODO.
- Added `dc-scope` ABNF rule (space-separated list of `dc-id` values; strict subset of OAuth 2.0 scope per RFC 6749 Appendix A.4) to Terminology; added normative imports of `"client_id"` and `"response_type"` from RFC 6749.
- Added normative constraints to the asynchronous OAuth authorization endpoint parameter table: `urlAsyncUX` cross-referenced to settings endpoint; `providerId` required to conform to `dc-id`; `client_id` referenced to RFC 6749 Section 2.2; `redirect_uri` required to be an absolute URI per RFC 3986; `response_type` value constrained to `"code"` per RFC 6749 Section 3.1.1; `scope` required to conform to `dc-scope`; `properties` parameter names required to conform to `variable-name` syntax.
- Added normative `"redirect_uri"` import from RFC 6749 Appendix A, Section A.6 to Terminology (used across sync flow, async flow, and token endpoint).
- Split combined `"code"`/`"refresh_token"` request row into two separate rows with mutual-exclusion constraint; added normative RFC 6749 syntax references to all parameters of the OAuth token endpoint request table (`"code"` A.11, `"refresh_token"` A.17, `"redirect_uri"` A.6, `"grant_type"` A.10, `"client_id"` §2.2, `"client_secret"` A.2).
- Converted token endpoint response table from two-column to three-column (Property/Key/Description) format; added normative RFC 6749 syntax references to all response properties (`"access_token"` A.12, `"token_type"` A.13 with mandatory `"bearer"` value, `"expires_in"` A.14, `"refresh_token"` A.17).
- Added normative constraints to the asynchronous OAuth apply endpoint parameter table: `urlAPI` cross-referenced to settings endpoint; `providerId` and `serviceId` required to conform to `dc-id`; `host` A-label normalisation rule reinstated for token comparison; `properties` parameter names required to conform to `variable-name` syntax with DNS Provider as correct actor; `instanceId` required to conform to `dc-id`; corrected `"serviceName"` guard attribute from `"sharedProviderName"` to `"sharedServiceName"`.
- Added normative constraints to the asynchronous OAuth revert endpoint parameter table: `urlAPI` cross-referenced to settings endpoint; `providerId` and `serviceId` required to conform to `dc-id` and updated to use "reverted" wording; `host` A-label normalisation rule added for token comparison; `instanceId` required to conform to `dc-id` with clarified removal semantics (single instance vs. all instances).
- Added `dc-version` ABNF rule for the `version` field of the template definition (positive integer, no leading zeros, no fractional or exponent part), as a strict subset of the JSON `"number"` type per RFC 8259 Section 6; added normative import of the `"number"` rule from RFC 8259; added RFC 8259 as a normative reference.
- Added normative ABNF rules for all Template Record fields: `dc-record-type` (type enum covering named, IANA-registered, and RFC 3597 unknown types), `dc-essential` (conflict-detection treatment enum with normative default `"Always"`), `dc-txt-conflict-mode` (TXT conflict mode enum with normative default `"None"`), `dc-conflict-prefix` (printable ASCII, U+0021–U+007E), `dc-name-char` (Unicode Assignable code point, RFC 9839 §4.3, excluding `"%"` and `"@"`), `dc-host-tmpl` (template form for host/name: `"@"` apex shorthand or mix of literal `dc-name-char` and `%var%` expressions), `dc-pointsto-tmpl` (template form for pointsTo/data/target: same structure), resolved forms reference `domain-name` (RFC 5890) directly; `dc-ttl-tmpl` / `dc-ttl-value` (template and resolved forms for TTL), `dc-uint16-tmpl` / `dc-uint16-value` (template and resolved forms for priority, weight, port), `dc-srv-protocol` (underscore-prefixed transport), `dc-srv-service` (underscore-prefixed service name per RFC 6335); added normative references RFC 2782 and RFC 6335; added normative imports of `"srv-rdata"` (RFC 2782) and `"service-name"` (RFC 6335); fixed missing `"number"` import (RFC 8259) in Terminology.
- Replaced `dc-label-chars`, `dc-host-literal`, `dc-host-value`, and `dc-pointsto-value` ABNF rules with `dc-name-char` (Unicode Assignable code points excluding `"%"` and `"@"`) as the literal-character unit in `dc-host-tmpl` and `dc-pointsto-tmpl`; resolved values for host, name, pointsTo, and target now reference `domain-name` (RFC 5890) directly in the table prose.
