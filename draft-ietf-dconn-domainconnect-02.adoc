...
- Removed ABNF rules `dc-scope` and `dc-force`, and imported terms `"client_id"` and `"response_type"`, as these are specific to the asynchronous flow; removed informative reference to <<RFC7591>> (OAuth Dynamic Client Registration).
- Added <<reserved-for-extensions,format=title>> statement to <<terminology,format=title>>; marked the `"urlAsyncUX"` settings property and its <<iana-settings-properties,format=title>> registry entry as Reserved for back-compatibility with <<DC-SPEC>>.
- Added `"Reserved"` status value to the <<iana-settings-properties,format=title>> registry.
- Removed OAuth Client Secret Leakage from <<security-considerations,format=title>>; retained <<sec-variable-phishing,format=title>>.
- Re-described the `"syncBlock"` template field: the DNS Provider MUST refuse a synchronous apply request for this template and MUST return an appropriate error.
...
* [[[DC-SPEC,DC-SPEC]]] "Domain Connect Specification", Version 2.3 Rev. 67, Domain Connect Community, <https://github.com/Domain-Connect/spec/releases/tag/rev.67>.
