...
_Fully Specified Record Types_ — A, AAAA, CNAME, MX, TXT, SRV, NS — whose RDATA is broken into dedicated template fields. The complete set is managed by the IANA "Domain Connect Fully Specified Record Types" registry (see <<iana-fully-specified-types>>). +

_Computed Record Types_ — which instruct the DNS Provider to derive DNS records by a defined computation rather than write a verbatim record. `"SPFM"` is the Computed Record Type defined in this document. +

_Generic Record Types_ — any other IANA-registered RR TYPE mnemonic, or the `"TYPE"` form of <<RFC3597>>, whose RDATA is carried opaquely in the `"data"` field. +
...
Not all templates are susceptible; the risk is proportional to how much of a record's content is controlled by variables. Three mitigations are available to template authors and Service Providers: digitally signing apply requests (see the Section <<<<signing-procedure>>,format=title>>), using an alternative flow not prone to this risk and disabling the synchronous flow via `"syncBlock"`, and setting the `"warnPhishing"` flag to prompt the DNS Provider to display additional warnings to the user.
