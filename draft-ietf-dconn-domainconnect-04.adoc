...
Change control for the registries in this group -- that is, altering a registry's format or registration policy, or reassigning the meaning of an existing entry -- requires either a new Standards Track RFC updating this document or IESG Approval, per <<RFC8126>> Section 4.9. This is distinct from the registration policy of each registry, which governs new entries only.
...
* *Handling*: how the DNS Provider processes a record of this type, as defined in <<terminology>>. The value MUST be one of `"Fully Specified"` or `"Computed"`. Generic Record Types are carried opaquely and are not registered here. A `"Reserved"` entry MAY leave this field unassigned.
...
- Reformulated the IANA registration procedure as guidance to the designated experts, replacing normative keywords and IANA-directed process with advice on when to approve a registration, and separately clarified the change control policy governing the registries themselves.
