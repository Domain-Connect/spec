...
A Domain Connect template is a declarative JSON document. It contains no executable content and no scripting or macro facility, and processing a template does not cause code to be executed on the recipient's system. A template does, however, describe DNS records that a DNS Provider is being asked to add to a user's zone, and applying one changes the configuration of that domain. A recipient therefore does not act on a template solely because it has been received; see <<sec-template-onboarding>> for how a DNS Provider establishes that a template is legitimate before accepting it, and <<user_authorization_of_changes>> for the authorization required before records are written.
...
Security considerations::: See <<sec-template-format>> and <<security-considerations>> of this document.
