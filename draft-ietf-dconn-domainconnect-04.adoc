...
* Ignore unknown: a recipient MUST ignore any element it does not recognise, and the mere presence of an unrecognised element MUST NOT cause the message, template, or request to be rejected. This applies to template-definition properties (see <<template-property-extensibility>>), template record fields (see <<template-record-extensibility>>), Settings End-Point properties (see <<settings-property-extensibility>>), and apply parameters (see <<apply-parameter-extensibility>>).
* *Must understand*: where an extension extends a message such that silently ignoring added elements would be unsafe, the sender marks the affected element names in a `"crit"` property, and a recipient that does not understand a listed element MUST reject the message rather than process it partially. This mechanism is defined for template definitions (see <<template-property-extensibility>>), for individual template records (see <<template-record-extensibility>>), and for the Settings End-Point (see <<settings-property-extensibility>>).

Consequently, an extension that can safely be ignored requires no version change; an extension that cannot safely be ignored is signalled per-message through `"crit"`; and only a change that breaks the framing of the protocol itself requires a new version path segment.
...
The `"crit"` property overrides the ignore-unknown default for the properties it lists: if the Service Provider does not understand and cannot process one or more of those properties, it MUST NOT proceed with the flow against this DNS Provider. A listed name that does not correspond to a property present in the settings response is by definition understood. As stated in <<versioning>>, `"crit"` MUST NOT be relied upon as a security mechanism, since a Service Provider implementing an earlier revision of this specification ignores the `"crit"` property itself.
