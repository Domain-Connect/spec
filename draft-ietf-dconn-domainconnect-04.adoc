...
dc-crit-list   =  dc-apply-param *( "," dc-apply-param )
                ; comma-separated list of apply parameter names;
                ; used for the crit parameter
...
dc-record-field =  dc-unprefixed-property / dc-vnd-property
                ; any template record field name

dc-settings-property =  dc-unprefixed-property / dc-vnd-property
                ; any settings-response property name
...
This specification is versioned at two independent levels: the HTTP interface and the template format.

All HTTP end-points defined in this document carry a `"/v2"` path segment immediately after the URL prefix obtained from discovery (see <<dns-provider-discovery>> and <<applying-domain-connect>>). The `"2"` denotes the protocol generation specified by this document.

A future revision of this protocol that is not backwards compatible with this document MUST use a different version path segment. A revision is not backwards compatible if an implementation of this document, when interacting with an implementation of that revision over the same path segment, could fail to interoperate or could misinterpret a request or a response. Reusing the `"/v2"` segment for such a revision MUST NOT be used. A deployment MAY serve more than one version path segment concurrently.

Revisions SHOULD instead strive to remain backwards compatible and be introduced as extensions under the existing `"/v2"` segment. To make this possible, every extensible data structure of this protocol follows the same principles:

* Ignore unknown: a recipient MUST ignore any element it does not recognise, and the mere presence of an unrecognised element MUST NOT cause the message, template, or request to be rejected. This applies to every extensible structure of this protocol: template-definition properties (see <<template-property-extensibility>>), template record fields (see <<template-record-extensibility>>), Settings End-Point properties (see <<settings-property-extensibility>>), apply parameters (see <<apply-parameter-extensibility>>), the response of the template query end-point (see <<query-supported-template>>), and the responses of the synchronous flow (see <<sync-apply-response>> and <<sync-apply-error-response>>).
* Must understand: where an extension extends a message such that silently ignoring added elements would be unsafe, the sender marks the affected element names in a `"crit"` element, and a recipient that does not understand a listed element MUST refuse to act on the message rather than process it partially. This mechanism is defined for template definitions (see <<template-property-extensibility>>), for individual template records (see <<template-record-extensibility>>), for the Settings End-Point (see <<settings-property-extensibility>>), and for apply parameters (see <<apply-parameter-extensibility>>).

Consequently, an extension that can safely be ignored requires no version change; an extension that cannot safely be ignored is signalled per-message through `"crit"`; and only a change that breaks the framing of the protocol itself requires a new version path segment. For the limits of `"crit"` against deployments that predate this specification, see <<sec-crit-legacy>>.

The must-understand mechanism is deliberately not defined for the responses of the synchronous flow. The authoritative record of what was applied is the DNS zone itself, which the Service Provider verifies independently (see <<verification-of-changes>>); by the time a response reaches the Service Provider the zone change has already occurred, so there is no operation left to refuse. Extensions to those responses therefore rely on the ignore-unknown rule alone.

The `"_domainconnect"` TXT record (see <<dns-provider-discovery>>) carries no internal structure and therefore offers no place to hold a version indicator. This is intentional in this revision to maintain compatibility with existing deployments of <<DC-SPEC>>. A future revision MAY define a structured content format for this record, at which point a version indicator can be introduced within that structure. Service Providers implementing this document MUST reject record contents that do not conform to the rules in <<dns-provider-discovery>>, which keeps any such future format distinguishable from a plain authority-and-path value.

The template format is versioned independently of the HTTP interface, through the `"templateSpecVersion"` template property (see <<template-property-extensibility>>).
...
(OPTIONAL) A list of the names of fields of this record that an implementation is required to understand and process for the record to be applied correctly. +

Each entry MUST conform to the `dc-record-field` syntax (see <<terminology>>). +

When present, the `"crit"` field MUST NOT be empty and MUST NOT contain variables.
--

The following table lists the fields that are defined for each record type by this document. Fields beyond those listed are governed by <<template-record-extensibility>>.
...
The set of template record fields defined above MAY be extended. Record field name space is partitioned as follows: 

- unprefixed names are reserved for this specification and for future standards-track extensions that normatively require this document as their base
- names beginning with `"vnd."` are reserved for vendor-specific and private use. 

Every record field name MUST conform to the `dc-record-field` syntax (see <<terminology>>).

Record fields fall into two categories. Protocol-level fields — `"type"`, `"groupId"`, `"essential"` and `"crit"` — are defined by this protocol and are applicable to a record irrespective of its `"type"`. Because such a field must carry the same meaning for every record type, unprefixed protocol-level field names are coordinated through the IANA "Domain Connect Protocol-Level Record Fields" registry (see <<iana-record-fields>>). All remaining fields are type-specific: they carry the RDATA of a particular record type or are otherwise meaningful only for certain types.

The two categories MUST NOT collide. A protocol-level field name, being valid for every record type, MUST NOT reuse a name already used as a type-specific field by any record type, and a specification defining type-specific fields MUST NOT reuse a name registered as protocol-level (see <<iana-record-fields>>).

An implementation MUST ignore any record field it does not recognise, and MUST ignore any recognised field that is not defined for the record's type. The mere presence of such a field MUST NOT cause the record or the template to be rejected.

The `"crit"` record field (see <<record-crit>>) overrides this default for the fields it lists: if an implementation does not understand and cannot process one or more of those fields, it MUST reject the template and MUST NOT apply it. Rejection applies to the template as a whole, not only to the record carrying the `"crit"` field, because a partially applied template would leave the service configured incorrectly. A listed name that does not correspond to a field present in the record is by definition understood.
...
|crit
|List of String
|(OPTIONAL) A list of the names of settings properties that a Service Provider is required to understand and process in order to interact correctly with this DNS Provider. +

Each entry MUST conform to the `dc-settings-property` syntax (see <<terminology>>). +
...
[[settings-property-extensibility]]
==== Settings Property Extensibility
...
Settings property names schema is partitioned as follows:

- unprefixed names are reserved for this specification and for future standards-track extensions that normatively require this document as their base,
- names beginning with `"vnd."` are reserved for vendor-specific and private use.

Every settings property name MUST conform to the `dc-settings-property` syntax (see <<terminology>>).

The `"crit"` property overrides the ignore-unknown default for the properties it lists: if the Service Provider does not understand and cannot process one or more of those properties, it MUST NOT proceed with the flow against this DNS Provider. A listed name that does not correspond to a property present in the settings response is by definition understood.
...
The Service Provider MUST ignore any JSON property in this response that it does not recognise; the presence of such a property MUST NOT cause the response to be treated as an error. Where the DNS Provider returns the full template object, the rules of <<template-property-extensibility>> and <<template-record-extensibility>> apply to it.
...
|*Critical Parameters*
|crit
|(OPTIONAL) A comma-separated list of the names of apply parameters that the DNS Provider is required to understand and process for the request to be handled correctly. +

The value MUST conform to the `dc-crit-list` syntax (see <<terminology>>). +

See <<apply-parameter-extensibility>> below.
|=======================================================================
...
The `"crit"` parameter overrides this default for the parameters it lists. If the DNS Provider does not understand and cannot process one or more of the listed parameters, it MUST NOT apply the template, and MUST signal an error as described in <<sync-apply-error-response>> using the error code `"invalid_request"`. A listed name that does not correspond to a parameter present in the request is by definition understood.
...
The DNS Provider MAY append parameters beyond those specified above to the `"redirect_uri"`. The Service Provider MUST ignore any parameter on the redirect that it does not recognise; the presence of such a parameter MUST NOT cause the Service Provider to treat an otherwise successful outcome as a failure. The same applies to the error redirect described in <<sync-apply-error-response>>.

The Service Provider SHOULD verify the outcome via DNS regardless of how the flow terminates (see <<verification-of-changes>>).


[[sync-apply-error-response]]
...
[[sec-crit-legacy]]
=== Must-Understand Markers and Pre-Existing Deployments

The `"crit"` must-understand marker (see <<versioning>>) is effective only against implementations that recognise `"crit"` itself. Deployments predating this specification, including those implementing <<DC-SPEC>> alone, ignore unrecognised elements without inspecting `"crit"`, and therefore proceed as though no such marker were present.

A sender consequently cannot infer from the absence of a rejection that the receiver honoured a `"crit"` marker. Where an extension conveys a security property — for example a constraint narrowing what a template may modify, or an assertion the receiver is expected to enforce — marking it in `"crit"` does not by itself prevent a receiver that predates the extension from applying the request without that property. `"crit"` MUST NOT be treated as a downgrade-prevention mechanism, and the security of an extension MUST NOT rest on the receiver's recognition of `"crit"` alone.
...
Values are registered after a three-week review period on the <dconn@ietf.org> mailing list or its successor, on the advice of one or more designated experts. To allow for the allocation of values prior to publication, the designated experts MAY approve registration once they are satisfied that the corresponding specification will be published.

Registration requests sent to the review mailing list SHOULD use an appropriate subject line (e.g., "Request to register Domain Connect record type: example"). Registration requests that remain undetermined for a period longer than 21 days MAY be brought to IANA's attention (using the <iana@iana.org> mailing list) for resolution.
...
* For property names without any prefix: Specification Required <<RFC8126>>. Registration additionally requires a standards-track RFC that normatively requires this document as its base.
* For property names beginning with `"vnd."`: Specification Required <<RFC8126>>.

Each entry in the registry MUST include:

* *Property name*: the JSON key as it appears in the settings response. The value MUST conform to the `dc-settings-property` syntax (see <<terminology>>).
* *Status*: the lifecycle state of the registration. The following values are defined; IANA MAY define additional values as needed:
...
This registry covers the properties of the template definition (see <<template-definition>>).
...
[[iana-record-fields]]
=== Domain Connect Protocol-Level Record Fields Registry

IANA is requested to create a new registry named "Domain Connect Protocol-Level Record Fields" under the "Domain Connect Protocol" registry group (see <<iana-settings-properties>>).

This registry covers only those template record fields that are protocol-level, that is, fields whose meaning is defined by this protocol and that are applicable to a template record irrespective of its `"type"` (see <<template-record-extensibility>>). Fields that carry the RDATA of a particular record type, or that are otherwise meaningful only for specific record types, are not registered here: the `"type"` field partitions that part of the name space, and each record type is free to define its own type-specific fields in the specification that defines the type.

Because a protocol-level field applies to records of every type, it occupies its name across all record types at once. A specification registering a protocol-level field MUST therefore establish that the requested name does not collide with a type-specific field name already in use by any record type, whether or not that type-specific field is itself registered. A name that collides in this way MUST NOT be registered, and a different name chosen.

Registration policy:

* For field names without any prefix: Specification Required <<RFC8126>>. Registration additionally requires a standards-track RFC that normatively requires this document as its base.
* For field names beginning with `"vnd."`: Specification Required <<RFC8126>>.

Each entry in the registry MUST include:

* *Field name*: the JSON key of the field as it appears in a template record. The value MUST conform to the `dc-record-field` syntax (see <<terminology>>).
* *Status*: the lifecycle state of the registration. The following values are defined; IANA MAY define additional values as needed:
** `"Active"`: the field is currently in use and its definition is normative.
** `"Deprecated"`: the field SHOULD NOT be used in new templates; it MAY appear in existing templates for backwards compatibility.
** `"Reserved"`: the field appears in earlier versions of the Domain Connect specification (see <<DC-SPEC>>) and is reserved for back-compatibility. It MUST NOT be redefined in a manner incompatible with <<DC-SPEC>>. See <<reserved-for-extensions>>.
* *Kind*: the nature of the defining document. The value MUST be one of: `"IETF Standard"` for fields defined in a standards-track RFC, `"Informational"` for fields defined in an Informational RFC, or `"Other"` for any other specification.
* *Reference*: the document that defines the field.

The following protocol-level record fields from this specification constitute the initial contents of the registry:

[cols=",,,"options="header",title="Initial Domain Connect Protocol-Level Record Fields"]
|=======================================================================
|*Field Name*
|*Status*
|*Kind*
...
- Partitioned the name spaces of apply parameters, template properties, template record fields, and settings properties into standards-track and vendor-private trees, and established IANA registries to coordinate them.
- Defined a common extensibility model for all protocol elements, combining an ignore-unknown default with a `"crit"` must-understand marker.
- Added <<versioning,format=title>>, describing how the protocol is versioned and how future revisions preserve backwards compatibility.
- Aligned the registration policies of all Domain Connect registries with the review process of <<iana-registration-procedure,format=title>>.
