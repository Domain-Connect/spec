...
The Domain Connect Protocol, as specified in <<draft-ietf-dconn-domainconnect>>, defines a synchronous flow for DNS configuration provisioning between Service Providers and DNS Providers.
...
Implementations of this extension MUST also implement <<draft-ietf-dconn-domainconnect>> however the implementation of synchronous flow is in this case OPTIONAL.
...
This document extends the settings response defined in <<draft-ietf-dconn-domainconnect>> by normatively
...
The `"urlAPI"` field defined in <<draft-ietf-dconn-domainconnect>> is also used for the asynchronous
...
(see `"multiInstance"` template property in <<draft-ietf-dconn-domainconnect>>). Allows for later removal of one template instance by DNS Providers storing this information. +
...
|instanceId
|(OPTIONAL) Only applicable to templates supporting multiple instances
(see `"multiInstance"` template property in <<draft-ietf-dconn-domainconnect>>). +
...
For DNS querying procedures (resolver selection, TTL considerations, retry intervals), refer to the Verification of Changes section in <<draft-ietf-dconn-domainconnect>>.
...
The phishing risks described in the Template Variable Phishing section of <<draft-ietf-dconn-domainconnect>> apply equally to the asynchronous flow.
...
The mitigations described in <<draft-ietf-dconn-domainconnect>> (disabling the synchronous flow via `"syncBlock"` and digitally signing apply requests) apply to the asynchronous flow as well. As in the synchronous flow, a template lacking such a mitigation is treated as susceptible to phishing by default.

== IANA Considerations

IANA is requested to update the `"urlAsyncUX"` entry in the "Domain Connect Settings
Properties" registry (created by <<draft-ietf-dconn-domainconnect>>) as follows:
...
* [[[draft-ietf-dconn-domainconnect, IETF(I-D.draft-ietf-dconn-domainconnect-02)]]] Kowalik, P., Blinn, A, Kolker, J., and S. Kerola, "Domain Connect Protocol - DNS provisioning between Services and DNS Providers", June 2026, <https://datatracker.ietf.org/doc/html/draft-ietf-dconn-domainconnect-02>.
