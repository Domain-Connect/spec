...
This document extends that protocol with an asynchronous OAuth 2.0 flow, needed by Service Providers that have more complex configurations that may require multiple steps and/or are asynchronous from the user's interaction.
...
This document extends the settings response defined in <<DOMAINCONNECT-BASE>> by normatively
...
The following field is added to the settings data structure defined in <<DOMAINCONNECT-BASE>>:
...
The `"urlAPI"` field defined in <<DOMAINCONNECT-BASE>> is also used for the asynchronous
...
=== General information
////
Moved to Introduction: "Using the OAuth flow is a more advanced use case needed by Service
Providers that have more complex configurations that may require multiple steps and/or are
asynchronous from the user's interaction."

Duplicate of Introduction (lines 66-68): "Not all DNS Providers will support the asynchronous
flow. As such it is recommended that Service Providers relying on an OAuth implementation also
...
For DNS querying procedures (resolver selection, TTL considerations, retry intervals), refer to the Verification of Changes section in <<DOMAINCONNECT-BASE>>.
...
The phishing risks described in the Template Variable Phishing section of <<DOMAINCONNECT-BASE>> apply equally to the asynchronous flow.
...
The mitigations described in <<DOMAINCONNECT-BASE>> (disabling the synchronous flow via `"syncBlock"`, digitally signing apply requests, and setting `"warnPhishing"`) apply to the asynchronous flow as well.

////
FRAGMENTS TO BE INCORPORATED INTO §8 (IANA Considerations).
These are original spec fragments preserved here pending step 10.

--- Fragment: IANA registry rows for urlAsyncUX ---
...
Properties" registry (created by <<DOMAINCONNECT-BASE>>) as follows:
...
=== draft-ietf-dconn-domainconnect-async-00
- Initial version. Defines the asynchronous OAuth 2.0 extension to the Domain Connect
  Protocol. Restores and restructures the asynchronous flow content previously present
  in draft-ietf-dconn-domainconnect-02, split into this companion document.
...
* [[[DOMAINCONNECT-BASE,DOMAINCONNECT-BASE]]] Kowalik, P., Blinn, A., Kolker, J., and S. Kerola, "Domain Connect Protocol - DNS provisioning between Services and DNS Providers", IETF Internet-Draft, draft-ietf-dconn-domainconnect (work in progress).
