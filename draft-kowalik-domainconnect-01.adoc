...
[horizontal]
Service Provider:: refers to entity that provide products and
services attached to domain names. Examples include web hosting
providers, email service providers and potentially even hardware manufacturers
with DNS-enabled devices like home routers or automation controls.

DNS Provider:: refers to entity that provide DNS services such as
registrars or standalone DNS services.
...
Apex Domain:: refers to a registered domain (e.g. example.com or
example.co.uk), or to a delegated zone in DNS.

Sub Domain:: refers to a sub-domain of an Apex Domain (e.g.
...
Discovery MUST work on the Apex Domain (zone) only. Bear in mind that
...
OAuth flows the valid values for the error parameter MUST be as
...
+
Under normal operation the `access_denied` error can be returned for a number of
...
of the DNS Provider and is therefore OPTIONAL.
...
|*Domain*
|domain
|(REQUIRED) The domain name being configured. This is the Apex Domain (the
registered domain or delegated zone).

|*Host*
|host
|(OPTIONAL) This is the host name of the sub domain. If left blank, the template is being
applied to the Apex Domain. Otherwise the template is applied to the sub domain of the
domain.
...
completion. It MUST be scoped to the `syncRedirectDomain` authority from the template, or the request MUST be signed.
...
|*Signing Algorithm*
|a
|(OPTIONAL) The parameter identifies the algorithm intended for
   use with the key. The values used SHALL be registered in the
   IANA "JSON Web Signature and Encryption Algorithms" registry
   established by <<RFC7518>>.
   If omitted will be assumed to be RS256.
   The support of RS256 is MANDATORY for both DNS Providers and Service
   Providers.

|*Public Key Format*
|t
|(OPTIONAL) The format of the public key.
If omitted will be will be assumed to be x509.
|=======================================================================
...
The DNS Provider SHOULD give the Service Provider a client id
and a secret which will be used when requesting tokens. For simplicity the client
id MAY be the same as the providerId, however it is up to the agreement
...
applied to the Apex Domain. For example:
...
|*Domain*
|domain
|(REQUIRED) The domain name being configured. This is the Apex Domain.
...
|providerName
|(OPTIONAL) This parameter allows for the caller to provide additional text for display
with the template providerName. This text SHOULD be used to augment the providerName value from the template, not replace it.

|*Service Name*
|serviceName
|(OPTIONAL) This parameter allows for the caller to provide additional text for display
with the template serviceName(s). It SHOULD be used to augment the serviceName value(s) from the template, not replace.
...
token, and a reasonable recommended setting is ten minutes, however the specific setup would depend on specifics of DNS Provider's implementation. As such this
...
Apex Domain or to any sub-domain of the Apex Domain that has been authorized.
...
|*Domain*
|domain
|(REQUIRED) The Apex Domain name being configured. It MUST match the domain that was authorized
in the token.

|*Host*
|host
|(OPTIONAL) The host name of the sub domain of the Apex Domain that was authorized in the
token. If omitted or left blank, the template is being applied to the Apex Domain.
...
|*Domain*
|domain
|(REQUIRED) The Apex Domain name being configured. It
MUST match the domain that was authorized in the token.

|*Host*
|host
|(OPTIONAL) The host name of the sub domain of the Apex Domain that was authorized in the token.
If omitted or left blank, the template is being applied to the Apex Domain.
...
DNS Providers may choose to maintain state inside records in DNS indicating the templates writing the records. Other providers may chose to not maintain this state.
...
For DNS Providers that maintain template state, an individual template is scoped to the set of records applied to a fully qualified domain. This includes the Apex Domain and the host (aka sub-domain) at apply time.
...
Some templates might want to utilize CNAME or TXT records with host values containing some form of user identification for validation of domain ownership, and these are often passed in variables. In those cases usage of variables in host variable can be applied without risk of issues mentioned above. In all other cases it is RECOMMENDED not to define variables in the `host` field.
...
- Changed term Root Domain to Apex Domain to align with <<RFC8499>>.
